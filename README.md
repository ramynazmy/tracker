...
the counts and a card per channel — its releases, and which features have moved in the last two
weeks. Open a release on a card to see its features as buttons: a filled dot is done, a ring is in
progress. Anything that has moved in the last two weeks is coloured, dated and sorted to the front,
and the count of those rides on the collapsed release row too. Releases shows how far each channel's releases have got; the Actions page lists
